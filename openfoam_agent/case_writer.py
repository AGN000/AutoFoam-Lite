...
from .schemas import CFDParams, TurbulenceModel, FlowRegime, GeometryType
...
class CaseWriter:
    def write_all(self, cfg: CaseWriterConfig) -> list[Path]:
        ...
        if cfg.has_gmsh_mesh:
            patches = _read_patch_names(case_dir)
        elif cfg.params.geometry_type == GeometryType.WEDGE:
            patches = ["inlet", "outlet", "symmetry", "wedge", "frontAndBack"]
        else:
        ...
        if p.has_heat_transfer:
            ...
            files["0/alphat"] = self._alphat_field(cfg, patches)
            if cfg.solver == "buoyantPimpleFoam":
                files["0/h"] = self._h_field_buoyant(cfg, patches)
...
"""
        elif cfg.solver == "buoyantPimpleFoam" and p.is_transient:
            # Adaptive timestepping keeps Co<0.5 and allows large steps at low velocity
            # avoiding the ~1500-step fixed-dt case that would far exceed 300s runtime
            extra = """\
adjustTimeStep  yes;
maxCo           0.5;
maxDeltaT       1;
...
"""
            ...
            write_interval = max(0.5, round(end / 5, 2))
        return h + f"""\
...
"""
        if p.is_compressible or solver == "buoyantPimpleFoam":
            extra_solvers += f"""\
    rho
    {{
        solver          diagonal;
...
"""
            ...
            # buoyantPimpleFoam uses absolute pressure (p_rgh reference must match
            # the operating pressure ~1e5 Pa; pRefValue=0 causes NaN in the pressure
            # solver normalization when the initial pressure RHS is near-zero)
            p_ref_value = "1e5" if solver == "buoyantPimpleFoam" else "0"
            algo = f"""\
PIMPLE
{{
...
"""
    ...
    def _block_mesh_dict(self, cfg: CaseWriterConfig) -> str:
        p = cfg.params
        if p.geometry_type == GeometryType.BACKWARD_FACING_STEP:
            return self._block_mesh_bfs(cfg)
        if p.geometry_type == GeometryType.WEDGE:
            return self._block_mesh_wedge(cfg)
...
"""
    ...
    def _block_mesh_wedge(self, cfg: CaseWriterConfig) -> str:
        """5-degree axisymmetric wedge mesh for pipe flow.

        Geometry: thin sector of a pipe, ±2.5° around the x-axis.
          axis (r=0) ─── x ──→
          wall (r=R) runs parallel above the axis.
          front / back faces are the two wedge planes at ±2.5°.

        Vertex layout (z=0 = back face at -θ, z>0 = front face at +θ):
          v0,v4 : inlet axis (x=0, r=0) — same point, degenerate
          v1,v5 : outlet axis (x=L, r=0) — same point, degenerate
          v2    : outlet wall, front (+θ)
          v3    : inlet  wall, front (+θ)
          v6    : outlet wall, back  (-θ)
          v7    : inlet  wall, back  (-θ)
        Block: hex(0 1 6 7  4 5 2 3)
        """
        import math
        p = cfg.params
        L  = p.length
        R  = (p.diameter if p.diameter else p.width) / 2.0
        nx, nr = cfg.nx, cfg.ny

        theta = math.radians(2.5)
        Ry = round(R * math.cos(theta), 8)
        Rz = round(R * math.sin(theta), 8)

        hdr = _header("dictionary", "blockMeshDict")
        return hdr + f"""\
convertToMeters 1;
...
(
    (0    0      0    )   // v0  inlet axis
    ({L}  0      0    )   // v1  outlet axis
    ({L}  {Ry}   {Rz} )   // v2  outlet wall, front (+2.5 deg)
    (0    {Ry}   {Rz} )   // v3  inlet wall,  front
    (0    0      0    )   // v4  inlet axis (degenerate = v0)
    ({L}  0      0    )   // v5  outlet axis (degenerate = v1)
    ({L}  {Ry}   {-Rz})   // v6  outlet wall, back  (-2.5 deg)
    (0    {Ry}   {-Rz})   // v7  inlet wall,  back
);

blocks
(
    hex (0 1 6 7 4 5 2 3) ({nx} {nr} 1) simpleGrading (1 1 1)
);
...
"""
    ...
    def _block_mesh_bfs(self, cfg: CaseWriterConfig) -> str:
        """Two-block backward-facing step mesh.

        Geometry (2-D, z-empty):
          y=W  ┌──block0──┬────────block1────────┐
          y=h  │  inlet   │  downstream (top)    │
               │  channel ├──────────────────────┤
          y=0  └──step────┤  downstream (bottom) │
               x=0  x=Lu  └──────────────────────┘ x=L

        Block 0 : upstream channel  (x=0→Lu,  y=h→W)
        Block 1 : downstream top    (x=Lu→L,  y=h→W)
        Block 2 : downstream bottom (x=Lu→L,  y=0→h)   ← recirculation zone
        """
        p = cfg.params
        L, W = p.length, p.width          # total length, total downstream height
        H = 0.001 if not p.is_3d else p.height
        h_step = W / 2.0                  # step height = half the downstream width
        # upstream length: 10 % of total, at least equal to step height
        Lu = max(h_step, round(L * 0.10, 4))
        Ld = L - Lu                       # downstream length

        nx, ny = cfg.nx, cfg.ny
        nx_u = max(4, round(nx * Lu / L))
        nx_d = nx - nx_u
        ny_u = max(2, ny // 2)            # cells spanning h→W
        ny_d = ny - ny_u                  # cells spanning 0→h

        hdr = _header("dictionary", "blockMeshDict")

        # 16 vertices: 8 at z=0, 8 at z=H
        # z=0 plane  (indices 0-7)
        # 0:(0,h,0)  1:(Lu,h,0)  2:(L,h,0)  3:(L,W,0)
        # 4:(Lu,W,0) 5:(0,W,0)  6:(Lu,0,0)  7:(L,0,0)
        # z=H plane  (indices 8-15, same xy order)
        def v(xi, yi): return f"({xi} {yi} 0)"
        def vh(xi, yi): return f"({xi} {yi} {H})"

        coords = [
            (0,    h_step), (Lu, h_step), (L,  h_step), (L,  W),
            (Lu,   W),      (0,  W),      (Lu, 0),       (L,  0),
        ]
        verts_z0 = "\n".join(f"    ({x} {y} 0)"   for x, y in coords)
        verts_zH = "\n".join(f"    ({x} {y} {H})" for x, y in coords)
...
blocks
(
    // block 0: upstream channel  (x=0→{Lu}, y={h_step}→{W})
    hex (0 1 4 5  8  9 12 13) ({nx_u} {ny_u} 1) simpleGrading (1 1 1)
    // block 1: downstream top    (x={Lu}→{L}, y={h_step}→{W})
    hex (1 2 3 4  9 10 11 12) ({nx_d} {ny_u} 1) simpleGrading (1 1 1)
    // block 2: downstream bottom (x={Lu}→{L}, y=0→{h_step})
    hex (6 7 2 1 14 15 10  9) ({nx_d} {ny_d} 1) simpleGrading (1 1 1)
);
...
(
    ...
    {{
        type patch;
        faces ((2 3 11 10) (7 2 10 15));
    }}
    walls
    {{
        type wall;
        faces (
            // inlet channel bottom (y=h, x=0→Lu)
            (0 1 9 8)
            // step face (x=Lu, y=0→h)
            (6 1 9 14)
            // bottom wall (y=0, x=Lu→L)
            (6 7 15 14)
            // top wall (y=W, x=0→Lu and Lu→L)
            (5 4 12 13)
            (4 3 11 12)
        );
    }}
    ...
    def _bc_block(self, patch: str, field: str, cfg: CaseWriterConfig) -> str:
        ...
        if field == "U":
            ...
            if u_bc == "fixedValue":
                ...
                return f"    {patch}\n    {{\n        type            fixedValue;\n        value           uniform ({U} 0 0);\n    }}\n"
            if u_bc == "inletOutlet":
                # External flows (cylinder, airfoil) can have backflow at the outlet;
                # internal flows (pipe, channel, BFS) should use zeroGradient for
                # clean outlet contours and to avoid spurious fixedValue artefacts.
                _external = p.geometry_type in (
                    GeometryType.CYLINDER, GeometryType.AIRFOIL,
                    GeometryType.SPHERE, GeometryType.AHMED_BODY,
                    GeometryType.WEDGE,
                )
                if _external:
...
"""
    ...
    def _h_field_buoyant(self, cfg: CaseWriterConfig, patches: list[str]) -> str:
        """0/h for buoyantPimpleFoam — sensible enthalpy h = Cp * T (Hf=0).
        Must be pre-initialised so heRhoThermo never sees h=0 → T=0 → FPE."""
        Cp = 1004.4
        T_ref, T_hot = 300.0, 350.0
        h_ref, h_hot = Cp * T_ref, Cp * T_hot
        hdr = _header("volScalarField", "h")
        bc_lines = []
        for patch in patches:
            lower = patch.lower()
            u_bc, _ = _get_patch_bc(patch)
            if u_bc in ("empty", "wedge"):
                ...
                bc_lines.append(f"    {patch}\n    {{\n        type            fixedValue;\n        value           uniform {h_hot};\n    }}\n")
            elif "wall" in lower or "fixed" in lower:
                bc_lines.append(f"    {patch}\n    {{\n        type            fixedValue;\n        value           uniform {h_ref};\n    }}\n")
            else:
                bc_lines.append(f"    {patch}\n    {{\n        type            zeroGradient;\n    }}\n")
        return hdr + f"""\
