...
def _bc_valid(case_dir: Path) -> bool:
    ...
    if not zero_dir.exists():
        return False
    has_inlet = has_outlet = has_any_bc = False
    for f in zero_dir.iterdir():
        ...
        if "outlet" in text and "type" in text:
            has_outlet = True
        # Closed-domain cases (LDC, cavity, box) have no inlet/outlet but are valid
        if "boundaryField" in text and "type" in text:
            has_any_bc = True
    # Accept cases with explicit inlet+outlet OR closed-domain BCs (movingWall, noSlip, etc.)
    return (has_inlet and has_outlet) or has_any_bc
    ...
    for line in log.splitlines():
        ...
        if "continuity errors" in low or "continuity" in low:
            for tok in line.split():
                tok = tok.rstrip(",")  # OpenFOAM uses "value, next=" format
                try:
                    v = float(tok)
                    if abs(v) < 1e-3:
                        return True
