...
OUTPUT
- Output ONLY a raw JSON object. No prose, no markdown, no ```json fences, no extra keys.
- Use EXACTLY these field names (snake_case): geometry_type, is_3d, length, width, height, diameter, reynolds_number, inlet_velocity, outlet_pressure, kinematic_viscosity, density, flow_regime, turbulence_model, is_transient, is_compressible, has_heat_transfer, is_multiphase, end_time, angle_of_attack, extraction_notes.
- Include every field listed above — no more, no fewer.
...
GEOMETRY DIMENSION FIELD MAPPINGS (which CFDParams field carries each dimension)
- cd_nozzle:  width = inlet diameter, diameter = throat diameter, height = outlet diameter,
    length = total nozzle length. (if user does not specify outlet, it defaults to 1.5 × inlet)
- elbow:      width = duct width/diameter, length = total duct path length.
- t_junction: width = duct width, length = main-leg length.
- diffuser:   width = inlet duct width (full height), length = diffuser length.
- s_bend:     width = duct width (half-height = width/2), length = total duct length.
- wedge:      length = chord, angle_of_attack = half-angle in degrees
    (e.g. "10 degree wedge" → angle_of_attack=10; default 10° if unspecified).
- airfoil:    length = chord, angle_of_attack = angle of attack in degrees.
- backward_facing_step: width = downstream channel height, length = total domain length.
- channel / box: width = cross-stream height, length = streamwise length.
- sphere:     diameter = sphere diameter; is_3d = true.
- ahmed_body: length = body length, width = body width, diameter = body height,
    angle_of_attack = rear slant angle in degrees (default 25°).
...
def extract(llm, refined_prompt: str, original_prompt: str | None = None) -> CFDParams:
    try:
        ...
        # CPU mode: no structured output — parse JSON from free text below
        class _SP:
            temperature = 0.0
            max_tokens = 2048
        sampling = _SP()
    ...
    # Extract the first JSON object from the response (handles free-text wrapping
    # when structured output is not available on CPU).
    import re as _re
    raw_json = raw_text
    ...
    _known = set(CFDParams.model_fields.keys())
    raw_dict = {k: v for k, v in json.loads(raw_json).items() if k in _known}
...
_GEOMETRY_DIMS: dict[str, dict] = {
    ...
    "cd_nozzle":     {"length": 3.0,   "width": 1.0,   "diameter": 0.5, "height": 0.001},
...
_LEN_PATTERNS = {
    ...
    "diameter": _re_module.compile(rf"(?:diameter|d\s*[=:]|d\s*of)\s*[=:]?\s*{_LEN_NUM}\s*(mm|cm|m|meters?)\b",
                                     _re_module.IGNORECASE),
    # CD nozzle outlet diameter — "outlet 2 m", "outlet diameter 2m", "2 m outlet"
    "outlet": _re_module.compile(
        rf"(?:outlet\s+(?:diameter|width|d)?\s*(?:[=:]|of|is)?\s*{_LEN_NUM}\s*(mm|cm|m|meters?)"
        rf"|{_LEN_NUM}\s*(mm|cm|m|meters?)\s+outlet)",
        _re_module.IGNORECASE),
    # CD nozzle throat — "throat 0.75 m", "throat diameter 0.5m"
    "throat": _re_module.compile(
        rf"throat(?:\s+(?:diameter|width|d))?\s*(?:[=:]|of|is|~)?\s*{_LEN_NUM}\s*(mm|cm|m|meters?)\b",
        _re_module.IGNORECASE),
    # CD nozzle inlet — "inlet diameter 1 m", "inlet 1 m"
    "inlet_d": _re_module.compile(
        rf"inlet(?:\s+(?:diameter|width|d))?\s*(?:[=:]|of|is|~)?\s*{_LEN_NUM}\s*(mm|cm|m|meters?)\b",
        _re_module.IGNORECASE),
}
...
def _extract_dim_from_prompt(prompt_lower: str, key: str) -> float | None:
    """Pull a length value (in metres) for 'chord', 'diameter', or 'outlet' from the prompt."""
    ...
    if not m:
        return None
    # outlet pattern has two alternatives (groups 1+2 or groups 3+4)
    if key == "outlet":
        if m.group(1) is not None:
            val_s, unit = m.group(1), m.group(2)
        else:
            val_s, unit = m.group(3), m.group(4)
    else:
        val_s, unit = m.group(1), m.group(2)
    try:
        v = float(val_s)
...
def _post_validate_raw(raw: dict, prompt: str = "", original_prompt: str | None = None) -> CFDParams:
    ...
    # For sphere / ahmed_body external flow: LLM tends to extract tiny dimensions
    # (0.001 m) when the user doesn't state domain size. Override with the
    # defaults from _GEOMETRY_DIMS whenever the extracted value is unrealistically
    # small for an external-flow domain.
    if geom in ("sphere", "ahmed_body"):
        _min_ext = 0.5  # minimum plausible domain side length in metres
        if length < _min_ext:
            length = dims["length"]
    ...
    # end_time: use LLM value if it looks like a deliberate transient duration
    # (< 500 s). LLM commonly returns 1000 for transient cases — that is the
    # steady-state pseudo-time default and should be discarded.
    raw_end_time = raw.get("end_time")
    if raw_end_time and raw_end_time > 0 and not (is_transient and raw_end_time >= 500):
    ...
    # cd_nozzle: regex-override inlet diameter (→width) and throat diameter (→diameter)
    # The LLM frequently puts the throat value into width, so we always trust regex here.
    if geom == "cd_nozzle":
        inlet_d = _extract_dim_from_prompt(_user_prompt_for_dims, "inlet_d")
        if inlet_d and inlet_d > 0:
            width = inlet_d
        throat_d = _extract_dim_from_prompt(_user_prompt_for_dims, "throat")
        if throat_d and throat_d > 0:
            diameter = throat_d
    ...
    if is_3d:
        ...
        elif geom in ("sphere", "ahmed_body"):
            # Enforce minimum external-flow domain height
        ...
        # For cd_nozzle, height carries the outlet diameter (> 0.01 distinguishes
        # it from the default 2D slab depth of 0.001).
        # Priority: regex from raw prompt → LLM-extracted height → default 0.001.
        outlet_d = _extract_dim_from_prompt(_user_prompt_for_dims, "outlet")
        if outlet_d and outlet_d > 0.01:
            height = outlet_d
        else:
            raw_h = raw.get("height")
            height = raw_h if (raw_h and raw_h > 0.01) else 0.001
    else:
        height = 0.001
    ...
    if geom in ("pipe", "cylinder", "sphere") and diameter:
        ...
        char_len = length
    elif geom in ("diffuser", "s_bend", "elbow", "t_junction", "cd_nozzle"):
        # Inlet width (hydraulic diameter) is the Re characteristic length
        char_len = width if width > 0 else length
    else:
    ...
    # Compressible flows: if still no meaningful velocity, infer from Mach number.
    # The LLM frequently returns inlet_velocity=0 when only Mach is specified.
    if is_compressible and inlet_velocity <= 0:
        import re as _re_mod
        mach_m = _re_mod.search(r'\b(?:mach|ma)\s*(?:number\s*)?[=:]?\s*(\d+\.?\d*)', prompt_lower)
        if mach_m:
            mach_val = float(mach_m.group(1))
            inlet_velocity = mach_val * 347.0  # speed of sound in air at ~300 K
    ...
    # Width: enforce minimum for external-flow domains
    if geom in ("sphere", "ahmed_body") and width < 0.5:
        width = dims["width"]

    # Diameter for pipe/cylinder/sphere
    if diameter is None or diameter <= 0:
        if geom in ("pipe", "cylinder"):
            diameter = width if geom == "pipe" else _get("diameter", dims.get("width", 0.1))
        elif geom == "sphere":
            diameter = 0.1  # default 100 mm sphere
