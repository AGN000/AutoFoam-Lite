...
def compute_time_settings(params: CFDParams, solver: str) -> dict:
    ...
    if solver == "buoyantSimpleFoam":
        return {"end_time": 2000, "delta_t": 1, "write_interval": 200}
    ...
    # Cap total timesteps for validation feasibility (600 s sim_timeout).
    # rhoPimpleFoam uses adjustTimeStep with maxDeltaT=1e-4 in controlDict,
    # so the effective dt is 1e-4 regardless of delta_t above.
    MAX_STEPS = 1500
    if solver == "rhoPimpleFoam":
        end_time = min(end_time, 1e-4 * MAX_STEPS)
    else:
        end_time = min(end_time, delta_t * MAX_STEPS)
